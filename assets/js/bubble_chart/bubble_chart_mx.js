...
function bubbleChartMX() {
  ...
  var center = { x: width / 2.3, y: height / 2 };

  var yearCenters = {
    24: { x: width / 1.7, y: height / 2 },
    25: { x: width / 3.2, y: height / 2 },
    22: { x:  4.3 * width / 5, y: height / 2},

    23: { x: width /2.3, y: height / 2 },
    10: { x: 2.2 * width / 3, y: height / 2 },
    1: { x: width, y: height },
  ...
  var fillColor = d3.scaleOrdinal()
    .domain(['Entertainment', 'Music', 'People & Blogs','Comedy','News & Politics','Film & Animation','Autos & Vehicles','Pets & Animals','Sports','Short Movies','Travel & Events','Gaming','Videoblogging','Howto & Style','Education','Science & Technology','Noneprofits','Movies','Anime/Animation','Action/Adventure','Classics','Comedy','Documentary','Drama','Family','Foreign','Horror','Sci-Fi/Fantasy','Thriller','Shorts','Shows','Trailers','0'])
    .range(['#F08080', '#8A2BE2', '#FFFF00', '#7CFC00','#00FFFF','#7FFFD4','#FF4500','#8B008B','#FFB6C1','#FFF8DC','#BC8F8F','#778899','#808080','#D3D3D3','#D2691E','#FF1493','#000080','#87CEFA','#90EE90','#008000','#FFD700','#E9967A','#808000','#2F4F4F','#8FBC8F','#008080','#C71585','#FAEBD7','#8B4513','#191970','#00FA9A','#FFFFFF','#000000']); 
  ...
  function createNodes(rawData) {
    ...
    var myNodes = rawData.map(function (d) {
      return {
        ...
        value: +d.views,
        name: d.channel_title,
        org: d.title,
        group: d.Category_name,
        year: d.category_id,
        trending: d.trending_date,
        x: Math.random() * 900,
        y: Math.random() * 800
      };
    });
  ...
  function showDetail(d) {
    ...
    var content = '<span class="name">Channel Title: </span><span class="value">' +
                  ...
                  addCommas(d.value) +
                  '</span><br/>' +
                  '<span class="name">Category Name: </span><span class="value">' +
                  d.group +
                  '</span><br/>'+
                  '<span class="name">Trending Date(YY/DD/MM): </span><span class="value">' +
                  d.trending +
                  '</span>';
...
function displayMX() {
  ...
  var myBubbleChart = bubbleChartMX();
  
  d3.csv('data/bubble_chart_data/mx_cat_data.csv', function(error, data) {
    if (error) {
      console.log(error);
    }
