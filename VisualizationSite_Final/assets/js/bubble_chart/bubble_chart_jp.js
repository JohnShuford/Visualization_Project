...
function bubbleChartJP() {
  ...
  var yearCenters = {
    24: { x: width / 1.8, y: height / 2 },
    25: { x: width / 3.2, y: height / 2 },
    22: { x:  4.4 * width / 5, y: height / 2},

    23: { x: width /2.4, y: height / 2 },
    10: { x: 2.3 * width / 3, y: height / 2 },
    1: { x: width, y: height },
...
function displayJP() {
  ...
  var myBubbleChart = bubbleChartJP();
  
  d3.csv('data/bubble_chart_data/jp_cat_data.csv', function(error, data) {
    if (error) {
      console.log(error);
    }
